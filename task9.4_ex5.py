...
for line in fhandle:
	...
	if words[0]=='From' and len(words)>2:	# To go to interested lines and guardian code for words[1]
		email=words[1]
		domain=email.split('@')				# double split to get domain seperately
		sample[domain[1]]=sample.get(domain[1],0)+1     # add the word in dictionary if it is first occurence, else add one to value each time it occurs
print(sample)
