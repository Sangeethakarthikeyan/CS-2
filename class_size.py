...
try:
	...
	print('Please enter valid numeric data only')
	sys.exit()

#Give a prompt for invalid inputs
if (year not in range(1,7)) or (acyear not in range(2004,2018)):
	print('Data not found. Invalid input. Please enter valid input')
	sys.exit()
...
for line in fhand:
	...
	if field[0]==str(acyear):	# For the required academic year
		print("The class size of Yr {} in the year {} is {}".format(year,acyear,field[year+1]))  # field[year+1] has the data for the required year 
